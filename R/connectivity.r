...
connectivity <- function (NET = "merged6_and_wir1_HC2", Lowercase = 1, col.1 = 1, col.2 = 2, col.score=3, echo = 1, main = "Connectivity plot", min.score = NA, n.top = NA, hex=FALSE) {
	if (is.list(NET)) {
		net.list <- NET;
	} else {
	...
	c0[which(c0 == 0)] = pseudoZero;
	h1 <- hist(c0, breaks = seq(from = min(c0-1, na.rm = TRUE), to = max(c0, na.rm = TRUE)+1, by = 1), plot = FALSE);
	Br = c(0, 2**seq(from = log2(min(c0, na.rm = TRUE)), to = log2(max(c0, na.rm = TRUE))+1, by = 1));
	t0 <- table(c0);
	# print(t0);
	...
	if (hex) {
		...
		intX[which(intX == log2(pseudoZero))] = 0;
		# print(intX);
	...
	legend("topright", legend = paste(c("Edge cut-off =", "N(top) =", "N(edges) =", "N(nodes) =", "gamma = "), 
		c(min.score, n.top, net.list$Ntotal, length(net.list$links), signif(Gamma, digits = 3)), collapse="\n"), bty = "n");
	if (!hex) {
		abline(coef = coef(Lm), col="red", lty = 2,untf = TRUE);
	}
	return(Gamma);
}
